...
| IaC | Terraform — Kind cluster + Helm releases |
| CI/CD | GitHub Actions → `ghcr.io` |
| Observability | kube-prometheus-stack (Prometheus + Grafana + Alertmanager) |
| Reliability | HPA v2 (CPU + memory), Locust load-test |

> **Team:** Adildabek Nurassyl, Artem Safaryan — group SE2408.

## Repository layout

```
.
├── app/                   # Go microservice (Dockerfile, main.go, go.mod)
├── terraform/             # IaC — kind cluster + helm releases (kps, metrics-server)
├── k8s/                   # Deployment, Service, HPA, ServiceMonitor, PrometheusRule
├── monitoring/
│   ├── alertmanager/      # AlertmanagerConfig (routes & receivers)
│   └── grafana/           # SRE Golden Signals dashboard
├── loadtest/              # Locust load test (staged spike profile)
├── scripts/               # bootstrap, teardown, smoke-test
├── .github/workflows/     # CI/CD pipeline (test → build → push → patch)
├── docs/                  # architecture.md, slo.md, runbook.md
└── REPORT.md              # Written report (render to PDF for submission)
```
...
|---|---|
| **IaC — providers, reproducibility, state** (15 pt) | `terraform/` + `terraform/README.md` |
| **CI/CD — build, registry, deploy** (15 pt) | `.github/workflows/ci-cd.yaml`, screenshot in `REPORT.md` |
| **Observability — metrics, dashboards, alerts** (15 pt) | `k8s/05-servicemonitor.yaml`, `k8s/06-prometheusrule.yaml`, `monitoring/grafana/`, `monitoring/alertmanager/` |
| **SRE Operations — SLOs, scaling, load tests** (15 pt) | `docs/slo.md`, `k8s/04-hpa.yaml`, `loadtest/` |
