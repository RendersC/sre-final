# SRE Capstone Project вЂ” Production Readiness Review
### `ecom-api`: a production-ready e-commerce microservice

**Course:** Site Reliability Engineering вЂ” Endterm & Final Exam
**Group:** SE2408
**Team:**
- **Adildabek Nurassyl** вЂ” IaC & Observability lead (Terraform, Prometheus, Grafana, Alertmanager)
- **Artem Safaryan** вЂ” Application & CI/CD lead (Go microservice, Dockerfile, GitHub Actions, HPA, load testing)

> The brief recommends groups of 3вЂ“4; our team is 2. Workload was scoped and
> re-balanced accordingly вЂ” see the per-section contribution notes at the end
> of every chapter.
...
**Repository:** `https://github.com/RendersC/sre-final` (insert your URL)
...
```
                в”Њв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ Developer laptop / Grader's laptop в”Ђв”ђ
                в”‚                                                              в”‚
                в”‚   Terraform в”Ђв–є Kind cluster (3 nodes)                        в”‚
                в”‚                  в”њв”Ђв”Ђ ns: ecom         (ecom-api Deployment   в”‚
                в”‚                  в”‚                     + Service + HPA       в”‚
                в”‚                  в”‚                     + ServiceMonitor      в”‚
                в”‚                  в”‚                     + PrometheusRule)     в”‚
                в”‚                  в””в”Ђв”Ђ ns: monitoring   (kube-prometheus-stack в”‚
                в”‚                                       Prometheus / Grafana /  в”‚
                в”‚                                       Alertmanager)          в”‚
                в”‚                                                              в”‚
                в”‚   GitHub Actions в”Ђв–є ghcr.io/<owner>/ecom-api:<sha> в”Ђв”Ђв–є pods  в”‚
                в”‚   Locust         в”Ђв–є http://localhost:30081 (NodePort)        в”‚
...
## 3. Step 1 вЂ” Infrastructure as Code (15 pt)
...
>
> *Successful `terraform apply` вЂ” 7 resources created.*

> ![kubectl get pods](docs/images/02-kubectl-get-pods.png)
>
> *`kubectl get pods -A` showing the cluster after Terraform finishes.*

**Contribution:** Adildabek (lead) вЂ” wrote and validated the Terraform stack;
Artem reviewed and added the metrics-server release.

---

## 4. Step 2 вЂ” Continuous Integration & Deployment (15 pt)
...
(`ghcr.io/<owner>/ecom-api:<sha>`) and additionally by `latest` for the
default branch.
...
2. `gcr.io/distroless/static-debian12:nonroot` is the final image вЂ” no shell,
...
**Contribution:** Artem (lead) вЂ” pipeline, Dockerfile, registry integration;
Adildabek validated end-to-end pull from cluster.

---

## 5. Step 3 вЂ” Observability & Alerting (15 pt)
...
|---|---|---|---|
| `http_requests_total` | counter | `route`, `method`, `status` | RED вЂ” Requests & Errors |
| `http_request_duration_seconds` | histogram | `route`, `method` | RED вЂ” Duration |
| `http_inflight_requests` | gauge | вЂ” | Saturation indicator |
| `orders_created_total` | counter | вЂ” | Business SLI |
| `orders_failed_total` | counter | вЂ” | Business SLI |
| `products_catalog_size` | gauge | вЂ” | Sanity / fixture check |

Scraping is wired through a `ServiceMonitor`
(`k8s/05-servicemonitor.yaml`) вЂ” the Prometheus Operator picks it up via the
...
Grafana and the alerts вЂ” this keeps the dashboard fast and the alert
...
`monitoring/grafana/dashboards/ecom-api-sre-dashboard.json` вЂ” the
**SRE Golden Signals** dashboard:

- Top row (single-stat panels): error rate, p95 latency, total request rate,
  Ready pod count вЂ” each with thresholds keyed to the SLO targets.
...
placeholder URL вЂ” in production the operator would substitute Slack or
PagerDuty endpoints.
...
**Contribution:** Adildabek (lead) вЂ” Prometheus rules, dashboard, Alertmanager
routing; Artem added the in-app metric instrumentation.

---

## 6. Step 4 вЂ” SRE Operations (15 pt)
...
|---|---|---|
| Availability (non-5xx ratio) | **99.5%** | в‰€ 3h 21m |
| Latency p95 (`/api/products*`) | **< 300 ms** | 5% of 5-min windows |
| Latency p95 (`/api/orders`) | **< 500 ms** | 5% of 5-min windows |
| Order success rate | **в‰Ґ 99%** | 1% of orders |

We also published an **error-budget policy** (`docs/slo.md` В§"Error-budget
policy") with four states from "ship freely" to "code freeze + page on-call".

### Auto-scaling

`k8s/04-hpa.yaml` вЂ” `HorizontalPodAutoscaler` v2:

- `minReplicas: 2`, `maxReplicas: 8`
- CPU target: 60% utilization
- Memory target: 75% utilization
- `behavior.scaleUp.policies` вЂ” add up to 2 pods per 30s; 30s stabilization
- `behavior.scaleDown.policies` вЂ” remove 1 pod per 60s; **180s stabilization**
...
2. A **staged spike** `LoadTestShape`:
   - 0вЂ“60s: ramp to 30 users
   - 60вЂ“180s: steady at 80 users
   - 180вЂ“300s: spike to 250 users (triggers HPA)
   - 300вЂ“420s: cool down to 80 users
   - 420вЂ“480s: drain to 10 users

While the spike runs we observe:
- Request rate climbs in Grafana
- HPA scales `ecom-api` from 2 в†’ 6 pods within ~90 seconds
- p95 stays under the SLO target except for a short transient at the start of
  the spike (before HPA reacts) вЂ” this is exactly the kind of evidence the
  error budget exists for

> ![HPA scaling](docs/images/08-hpa-scaling.png)
> ![Locust results](docs/images/09-locust-results.png)

**Contribution:** Artem (lead) вЂ” HPA tuning + Locust profile; Adildabek
validated SLO derivation and dashboard correlation during the spike.

---

## 7. Step 5 вЂ” Documentation & Repository Hygiene (10 pt)

- `README.md` вЂ” quick start, repo layout, rubric mapping
- `docs/architecture.md` вЂ” system diagram + design rationale
- `docs/slo.md` вЂ” SLI/SLO definitions + error-budget policy
- `docs/runbook.md` вЂ” playbook for every alert in the system
- `terraform/README.md` вЂ” how to reproduce / how to swap state backend
- `loadtest/README.md` вЂ” how to drive load and what to look for
- `.gitignore` вЂ” Terraform state, Go binaries, Python venv, IDE files
- Per-folder READMEs where it adds value

The repository is structured by concern (`app/`, `terraform/`, `k8s/`,
`monitoring/`, `loadtest/`, `docs/`, `scripts/`) вЂ” easy to navigate during the
defense.

---

## 8. Defense & Demo вЂ” Plan (30 pt)
...
| 2:30 | Open Prometheus в†’ Targets, then Alerts | `ecom-api` UP, alert rules loaded |
| 3:00 | Open Alertmanager UI | Empty (good) вЂ” explain receivers |
| 3:30 | Start `locust -f locustfile.py` with the staged spike | Traffic ramping in Grafana |
| 5:00 | `kubectl -n ecom get hpa,pods -w` next to Grafana | HPA scales from 2 в†’ 6 pods |
...
- **Adildabek** вЂ” IaC walkthrough (`terraform/` file by file), Prometheus
  Operator pattern, dashboard derivation from SLIs, Alertmanager routing.
- **Artem** вЂ” Go service instrumentation, Dockerfile (distroless, non-root),
...
| SLO burn-rate alerts (multi-window multi-burn) | Beyond the depth covered in lectures | Add Google SRE workbook recipes once we have в‰Ґ 28 days of metrics |

---

## 10. Appendix вЂ” Reproducibility checklist
...
- [ ] http://localhost:30030 loads Grafana, dashboard "ecom-api вЂ” SRE Golden Signals" is present
...
- [ ] `ghcr.io/<owner>/ecom-api:<sha>` is present in the packages tab
